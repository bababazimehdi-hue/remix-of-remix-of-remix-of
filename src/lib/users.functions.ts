...
/*
 * The former public `bootstrapFirstAdmin` endpoint was removed: it was an
 * unauthenticated account-creating endpoint (public registration). The first
 * OWNER is now created exclusively by the first-run initialization flow, which
 * grants the OWNER role through the one-time `initialize_system()` function.
 */
