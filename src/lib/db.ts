...
function purchaseFromRow(r: Row): BicyclePurchase {
  const out = {
    ...
    repairedAt: r.repaired_at ? iso(r.repaired_at) : undefined,
    repairedBy: r.repaired_by ?? undefined,
    repairedNote: r.repaired_note ?? undefined,
...
const purchaseToRow = (p: BicyclePurchase): Row => ({
  ...
  repaired_at: p.repairedAt ?? null,
  repaired_by: p.repairedBy ?? null,
  repaired_note: p.repairedNote ?? null,
});
...
function taskFromRow(r: Row): Task {
  const out = {
    ...
    repairType: (r.repair_type ?? "INTERNAL") as Task["repairType"],
    title: r.title ?? "",
    description: r.description ?? "",
...
const taskToRow = (t: Task): Row => ({
  ...
  repair_type: t.repairType ?? "INTERNAL",
  title: t.title,
...
export async function loadAll(viewerId: string | null): Promise<Partial<LoadedData>> {
  ...
  for (const r of (roles.data ?? []) as Row[]) {
    // The database enum has an extra `OWNER` level; in the app the highest
    // level is `ADMIN` (پشتیبان), so OWNER maps onto it.
    const raw = String(r.role);
    roleOf.set(r.user_id, (raw === "OWNER" ? "ADMIN" : raw) as Role);
  }
