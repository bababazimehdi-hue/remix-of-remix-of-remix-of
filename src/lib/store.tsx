...
export type User = {
  ...
  bio?: string;
  /** Personal profile picture (compressed data URL), editable by the account owner. */
  avatarUrl?: string;
};
...
export type BicyclePurchase = {
  ...
  repairTaskId?: string;
  /** Set once the bike moved to the «دوچرخه‌های تعمیر شده» section. */
  repairedAt?: string;
  repairedBy?: string;
  repairedNote?: string;
};
 ...
 */
export const isRepairedBike = (p: BicyclePurchase) => !!p.repairedAt;
  ...
  const value = useMemo<Ctx>(() => {
    ...
    return {
      ...
      login: async (identifier: string, password: string) => {
        ...
        // No public registration: unknown accounts are rejected. The only
        // account-creating paths are the first-run initialization flow (OWNER)
        // and the OWNER creating team members from inside the app.
