...
function SettingsPage() {
  ...
  // The whole «تنظیمات» section is reserved for the support account: the
  // system owner, or anyone holding the ADMIN (پشتیبان) role.
  const isSupport = isOwner || user.role === "ADMIN";

  if (ownerLoading && !isSupport) {
    return (
      <div className="grid min-h-[50vh] place-items-center">
        <span className="text-sm font-bold text-muted-foreground">در حال بررسی دسترسی…</span>
      </div>
    );
  }
  ...
  if (!isSupport) {
    return (
      <>
        <PageHeader title="تنظیمات" subtitle="دسترسی محدود" />
        <EmptyState
