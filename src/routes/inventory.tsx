...
import { BadgeCheck, Bike, Palette, Ruler, Search, Wrench } from "lucide-react";
...
import { markBikeRepaired } from "@/lib/repaired-bikes";
  ...
  const bikes = useMemo(
    () =>
      state.purchases.filter(
        (p) =>
          !isRepairedBike(p) &&
          (p.status === "APPROVED" || p.status === "SYNCED_TO_ACCOUNTING"),
      ),
    [state.purchases],
  );

  function moveToRepaired(bike: BicyclePurchase) {
    if (!user) return;
    setState((s) => ({ ...s, purchases: markBikeRepaired(s.purchases, bike.id, user.id) }));
                    ...
                    {isManager ? (
                      <>
                        ...
                        >
                          ...
                          انتقال به دوچرخه‌های تعمیر شده
                        </button>
                      </>
                    ) : null}
                  </div>
