...
import { markBikeRepaired, openBikeTasks } from "@/lib/repaired-bikes";
...
function TaskDetail() {
  ...
  function approve() {
    ...
    // Once every repair of a bike is finalised, the bike itself leaves the
    // purchases/inventory accounts and moves to «دوچرخه‌های تعمیر شده».
    const bikeId = task!.bikeId;
    if (bikeId) {
      const stillOpen = openBikeTasks(state.tasks, bikeId).filter((t) => t.id !== task!.id);
      if (!stillOpen.length) {
        setState((s) => ({
          ...s,
